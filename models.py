...
def load_eccv16():
    """Load ECCV16 model with pre-trained weights"""
    try:
        model = ECCVGenerator()
        state_dict = torch.hub.load_state_dict_from_url('https://colorizers.s3.us-east-2.amazonaws.com/colorization_release_v2-9b330a0b.pth', map_location='cpu', check_hash=True)
...
def load_siggraph17():
    """Load SIGGRAPH17 model with pretrained weights from torch.hub"""
    try:
        import os
        model = SIGGRAPHGenerator()
        
        # Load official pre-trained weights
        state_dict = torch.hub.load_state_dict_from_url('https://colorizers.s3.us-east-2.amazonaws.com/siggraph17-df00044c.pth', map_location='cpu', check_hash=True)
        
        # Load with strict=False because the official map has `model_class` layers we don't use
