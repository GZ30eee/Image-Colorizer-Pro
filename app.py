...
# --- SAFE GEMINI CONFIGURATION ---
GEMINI_API_KEY = None

# 1. Try from .env file (Highest priority for local development)
try:
    from dotenv import load_dotenv
    load_dotenv(override=True) # Force override of shell environment variables
    
    key = os.environ.get("GEMINI_API_KEY")
    if key:
        GEMINI_API_KEY = str(key).strip().strip("\"'") # ensure strictly clean
except ImportError:
    pass
...
if not GEMINI_API_KEY:
    key = os.environ.get("GEMINI_API_KEY")
    if key:
        GEMINI_API_KEY = str(key).strip().strip("\"'")
...
with st.sidebar:
    ...
    with st.expander("🧠 AI Models & Performance", expanded=True):
        st.info("💡 Light/Fast models are better for prototyping. SIGGRAPH17 is for final high-quality output.")
        # Device-aware recommendations
        model_descriptions = {
            'eccv16': 'ECCV 2016 - High quality, slower',
            'lightweight': 'Lightweight - Balanced quality/speed',
            'fast': 'Fast - Quick results',
            'siggraph17': 'SIGGRAPH 2017 - Vibrant, best quality',
            'deoldify': 'DeOldify (GAN) - Vibrant, stylistic (Pro)',
            'instcolorization': 'InstColorization - Object aware (Pro)',
            'chromagan': 'ChromaGAN - Rich semantic colors (Pro)'
        }
        
        selected_models = []
        for model_id, desc in model_descriptions.items():
            if st.checkbox(desc, value=(model_id in ['fast', 'siggraph17']), key=f"model_{model_id}"):
                selected_models.append(model_id)
        
        st.session_state.selected_models = selected_models
        
        # QUALITY CONTROL for SPEED Optimization
        processing_quality = st.select_slider(
            "Image Processing Resolution:",
            options=["Fast Preview (512p)", "Standard (720p)", "Max Quality (Original)"],
            value="Standard (720p)",
            help="Lower resolution significantly speeds up AI model processing."
        )
        st.session_state.processing_quality = processing_quality
    
    with st.expander("🎨 Magic & Advanced Features", expanded=False):
        use_color_hints = st.checkbox(
            "Enable Interactive Color Hints",
            value=True,
            help="Required to use points for SIGGRAPH17"
        )
        
        # Gemini Analysis
        if st.session_state.gemini_available:
            use_gemini_analysis = st.checkbox(
                "Enable Gemini Vision Analysis",
                value=True,
                help="Let AI suggest colors and eras"
            )
        else:
            use_gemini_analysis = False
            st.warning("⚠️ API key not found for Vision Analysis")
            
        color_vibrancy = st.slider("Global Color Vibrancy", 0.5, 2.0, 1.2, 0.1)
    ...
    # About section
    st.markdown("---")
    st.success("✨ **AI Image Colorizer Pro v2.0**\n\nOptimized for performance and aesthetic design.")
    ...
    if uploaded_file is not None:
        try:
            ...
            if is_video and processing_mode == "Video (Beta)":
                ...
                img_np_original = img_processor.load_image(uploaded_file.getvalue())
                
                # OPTIMIZATION: Resize image based on quality setting
                quality = st.session_state.get('processing_quality', "Standard (720p)")
                max_dim = None
                if quality == "Fast Preview (512p)":
                    max_dim = 512
                elif quality == "Standard (720p)":
                    max_dim = 720
                ...
                if max_dim is not None:
                    ...
                    if max(h, w) > max_dim:
                        ...
                        img_np = cv2.resize(img_np_original, (new_w, new_h), interpolation=cv2.INTER_AREA)
                    else:
            ...
            if st.session_state.image_uploaded:
                ...
                with col2:
                    ...
                    # Gemini Analysis if enabled
                    if use_gemini_analysis and st.session_state.gemini_available:
                        with st.spinner("Analyzing image with Gemini Vision..."):
                            try:
                                ...
                                response = model.generate_content(
                                    ["Analyze this image and suggest appropriate colors for different parts. Be specific about color suggestions for sky, buildings, nature, people, etc. Also estimate the era/period of the image if possible.", img_pil]
                                )
                ...
                with st.expander("🎛️ Manual Color Tuning", expanded=False):
                ...
                with process_col1:
                    if st.button("✨ Magic Auto-Colorize", type="primary", use_container_width=True):
                        with st.status("🔮 Magic in progress...", expanded=True) as status:
                            ...
                            try:
                                ...
                                st.write("Loading selected AI models...")
                                ...
                                for model_name, model in models.items():
                                    try:
                                        with torch.no_grad():
                                            ...
                                            # Apply temporal consistency for video (if single image, does nothing)
                                            out_ab_processed = out_ab
                                ...
                                if successful_models > 0:
                                    status.update(label=f"✅ Successfully processed {successful_models} model(s)!", state="complete", expanded=False)
                                    st.toast("✅ Processing complete!", icon="🎨")
                                else:
                                    status.update(label="❌ All models failed.", state="error", expanded=True)
                                
                            except Exception as e:
                                status.update(label=f"❌ Failed: {str(e)}", state="error", expanded=True)
                ...
                if st.session_state.model_results:
                    ...
                    if num_results > 0:
                        ...
                        # Draw Canvas for multi-point hints
                        if use_color_hints:
                            with st.expander("🖌️ Multi-Point Scribbles (Draw Color Hints)"):
                                st.info("Draw on the image to force specific colors. Works best with SIGGRAPH17.")
                                ...
                                canvas_result = st_canvas(
                                    ...
                                    background_image=Image.fromarray((img_processor.create_grayscale(img_np) * 255).astype(np.uint8)),
                                    update_streamlit=True,
                                    height=int(img_np.shape[0] * (400 / img_np.shape[1])),
                                    width=400,
                        ...
                        with st.expander("🔍 Compare Results Side-by-Side (Drag Slider!)", expanded=True):
                            st.info("💡 Drag the slider to compare the grayscale original with the models.")
                            
                            # Let user pick which model to compare against the grayscale
                            compare_model = st.selectbox(
                                "Select Model for Comparison:",
                                [str(m).upper() for m in result_keys],
                                index=result_keys.index('siggraph17') if 'siggraph17' in result_keys else 0
                            )
                            selected_compare_key = compare_model.lower()

                            # Convert numpy arrays to PIL images for the comparison tool
                            gray_pil = Image.fromarray((st.session_state.model_results['grayscale'] * 255).astype(np.uint8))
                            color_pil = Image.fromarray((st.session_state.model_results[selected_compare_key] * 255).astype(np.uint8))

                            image_comparison(
                                img1=color_pil,
                                img2=gray_pil,
                                label1=f"{compare_model} Colorized",
                                label2="Original Grayscale",
                                # Optional parameters
                                width=700,
                            ...
                            # Render basic side-by-side array below the interactive widget
                            st.markdown("##### Static Comparison Grid")
        ...
        with st.expander("📚 Example Use Cases"):
            ...
            with col2:
                ...
                st.markdown("Get intelligent color suggestions from Gemini")
...
with tab2:
    ...
    if st.session_state.performance_monitor.metrics_history:
        ...
        # Allow exporting metrics via Pandas CSV
        df = pd.DataFrame(metrics_data)
        st.download_button(
            label="📥 Export Metrics to CSV",
            data=df.to_csv(index=False).encode('utf-8'),
            file_name="colorization_metrics_export.csv",
            mime='text/csv'
        )
...
with tab3:
    ...
    if st.session_state.gemini_available:
        ...
        if st.session_state.ai_analysis and st.button("Generate Color Palette from Analysis"):
            try:
                # Ask Gemini to suggest a color palette
                model = genai.GenerativeModel('gemini-2.5-flash')
                response = model.generate_content(
                    f"You are a color expert. Based on this image analysis: {st.session_state.ai_analysis['text']}\n\nSuggest a color palette with 5 colors in hex format. Return only the hex codes separated by commas."
                )
                
                palette_hex = response.text.strip()
        ...
        st.warning("Gemini API is not configured. Please add your API key to use AI analysis features.")
        st.info("To enable AI analysis, set your GEMINI_API_KEY in Streamlit secrets or environment variables.")
...
with tab4:
    ...
    with col1:
        ...
        st.metric("Gemini Vision", "Available" if st.session_state.gemini_available else "Not Configured")
...
with footer_col4:
    st.markdown("**Gemini:** ✅ Available" if st.session_state.gemini_available else "**Gemini:** ⚠️ Not Configured")
